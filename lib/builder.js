...
Builder.prototype.getFullTextSearchClause = function(model, prefix, value){
...
Builder.prototype.build = function(sequelizeModel, options) {
  ...
  if (!_.isEmpty(this._where)) {
    if(!_.isEmpty(this._where['q'])){
      ...
      var fullTextOrCond = [this.getFullTextSearchClause(sequelizeModel, "`" + sequelizeModel.name + "`" + ".", value)];
      result.include = this._model.getFullTextIncludeModels(this._model.sequelize.models);
      // TODO: https://bitbucket.org/vidyahub/vidyahub/issues/42/how-does-one-pass-this-argument-to-lodash resolve
      this.getEmbeddedIncludeModels(result.include).forEach(function(model){
        if(model && model.model && model.model.getFullTextSearchColNames && _.isFunction(model.model.getFullTextSearchColNames)){
          fullTextOrCond.push(this.getFullTextSearchClause(model.model, model.prefix, value)); 
        }
      }, this);
      this._where = this.buildVirtualFieldWhereClauses(this._where, options);
      result.where = {$and: [this._where, {$or: fullTextOrCond}]};
    } else {
      result.where = this.buildVirtualFieldWhereClauses(this._where, options);
    }
  }
