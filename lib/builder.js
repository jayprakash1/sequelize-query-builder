  ...
  if(_.isArray(include)){
    var mappedArray = _.map(include, function(model){
      ...
      if(model.model){
        ...
        var existingModelIncludes = model.include; 
        ...
        if(model.limit){
          ...
          this._subquery = true;
          var res = this.getFullTextSearchClauses(model.model, false);
        ...
        var arr = [{model: model.model, prefix: this._model.sequelize.queryInterface.quoteIdentifier(tmpPrefix) + "."}].concat(this.getEmbeddedIncludeModels(existingModelIncludes, tmpPrefix + "." ));
        return arr;
      }
      else{
...
Builder.prototype.getFullTextSearchClauses = function(model, isStartModel){
...
Builder.prototype.build = function(sequelizeModel, options) {
  ...
  if (!_.isEmpty(this._where)) {
    ...
    if(!_.isEmpty(this._where['q'])){
      ...
      fullTextIncludeWhereClauses = this.getFullTextSearchClauses(sequelizeModel, true);
    }
