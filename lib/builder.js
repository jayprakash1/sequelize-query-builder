...
Builder.prototype.build = function(sequelizeModel) {
  ...
  if(this._include){
    ...
    this._include = _.forEach(this._include, function(obj){obj.model = sequelizeModel.sequelize.models[obj.model]});
    if(result.include == null){
      result.include = this._include;
    }else{
      // create hash with model names and then merge to avoid name clash in SQL generation
      var thisIncludeHash = _.indexBy(this._include, function(obj){return obj.model.name});
      var onlyResultInclude = _.filter(result.include, function(obj){
        if(thisIncludeHash[obj.model.name] == null){
          return true;
        } else if(obj.include) {
          // add include so that it is used later otherwise this include is missed
          thisIncludeHash[obj.model.name].include = obj.include; 
        }
      });
      result.include = onlyResultInclude.concat(this._include);
    }
  }
