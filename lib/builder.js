    ...
    var mappedArray = _.map(include, function(model){
      ...
      if(model.model){
        ...
        if(model.limit){
          ...
          // TODO: interesting that it is overwriting the required includes for the top model so commenting out for now
...
Builder.prototype.getFullTextSearchClauses = function(model, subQuery){
  ...
  if(subQuery){
    ...
    return {where: [model.getSubQueryFullTextSearchSQL(_.pluck(res.where.$or, 'val').join(" OR "))], include: []};
  } else {
    return res;
  }
}
