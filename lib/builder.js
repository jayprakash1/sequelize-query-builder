...
Builder.prototype.getEmbeddedIncludeModels = function(include, parentModel, prefix){
  if(prefix == null)
    prefix = "";
  ...
  if(_.isArray(include)){
    var mappedArray = _.map(include, function(includeModel){
      ...
      if(includeModel.model){
        ...
        if(includeModel.limit){
          ...
          this._subquery = true;
          // TODO: include should be a new tree now which should start at current node in already include tree for topmost model. this needs to implemented for generic case. For now just do once level
          var resInclude = [{model: parentModel, include: _.filter(include, function(tmpIncludeModel){return includeModel != tmpIncludeModel ? true : false})}];
          resInclude = resInclude.concat(includeModel.include);
          var res = this.getFullTextSearchClauses(includeModel.model, resInclude);
          includeModel.where = res.where;
          includeModel.include = includeModel.include;
        }
        ...
        var arr = [{model: includeModel.model, prefix: this._model.sequelize.queryInterface.quoteIdentifier(tmpPrefix) + "."}].concat(this.getEmbeddedIncludeModels(existingModelIncludes, includeModel.model, tmpPrefix + "." ));
        return arr;
      }
      else{
        tmpPrefix = prefix + includeModel.name;
        return [{model: includeModel, prefix: this._model.queryInterface.queryInterface(tmpPrefix) + "."}]
      }
    }, this);
    ...
    this.getEmbeddedIncludeModels([include], parentModel);
  }
}
...
Builder.prototype.getFullTextSearchClauses = function(model, include){
  ...
  this.getEmbeddedIncludeModels(res.include, model).forEach(function(includeModel){
...
Builder.prototype.getFullTextSearchClause = function(model, prefix){
  ...
  var whereClause = prefix + "`"+ model.getFullTextSearchColNames()[0] +"` LIKE '%" + this._search+"%' "

  for(i=1; i<model.getFullTextSearchColNames().length; i++)
    str = model.getFullTextSearchColNames()[i];
    whereClause += "OR " + prefix+"`"+str+"` LIKE '%" + this._search + "%' ";
  return this._model.sequelize.literal(whereClause);
    ...
    var mergeIncludeObjects = function(objArr1, objArr2){
      ...
      _.forEach(objArr1, function(obj){
        if(objArrHash2[obj.model.name] != null){
          ...
          if(obj.where && objArrHash2[obj.model.name].where)
            objArrHash2[obj.model.name].where = {$and: [obj.where, objArrHash2[obj.model.name].where]};
          else if(obj.where)
            objArrHash2[obj.model.name].where = obj.where;
