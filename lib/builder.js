...
Builder.prototype.buildVirtualFieldWhereClauses = function(whereNode, sequelizeModel, options){
  ...
  if(["$and", "$or"].indexOf(key) >= 0){
    whereNode[key][0] = this.buildVirtualFieldWhereClauses(whereNode[key][0], sequelizeModel, options); 
    whereNode[key][1] = this.buildVirtualFieldWhereClauses(whereNode[key][1], sequelizeModel, options);
    return whereNode;
  }
  if(_.isArray(sequelizeModel._virtualAttributes)
      && sequelizeModel._virtualAttributes.length > 0
      && sequelizeModel._virtualAttributes.indexOf(key) >= 0){
    return sequelizeModel.sequelize.where(sequelizeModel.attributes[key].rawQuery(options), whereNode[key]);
  } else {
    return whereNode;
  }
}
...
Builder.prototype.build = function(sequelizeModel, options) {
  ...
  if (!_.isEmpty(this._where)) {
    if(!_.isEmpty(this._where['q'])){
      ...
      this._where = this.buildVirtualFieldWhereClauses(this._where, sequelizeModel, options);
      ...
      result.where = this.buildVirtualFieldWhereClauses(this._where, sequelizeModel, options);
    }
  }
