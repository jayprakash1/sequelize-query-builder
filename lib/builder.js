    ...
    var mappedArray = _.map(include, function(includeModel){
      ...
      if(includeModel.model){
        ...
        if(includeModel.limit){
          ...
          var resInclude = {model: parentModel, include: _.filter(include, function(tmpIncludeModel){return tmpIncludeModel != includeModel ? true : false})};
          resInclude.push(includeModel.include);
          var res = this.getFullTextSearchClauses(includeModel.model, resInclude);
          includeModel.where = res.where;
          includeModel.include = resInclude;
        }
