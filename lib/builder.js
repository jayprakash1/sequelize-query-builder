...
Builder.prototype.build = function(sequelizeModel) {
  ...
  if (!_.isEmpty(this._where)) {
    if(!_.isEmpty(this._where['q'])){
      ...
      fullTextOrCond = [sequelizeModel.sequelize.literal("match(" + sequelizeModel.getFullTextSearchColNames().toString() + ") against('" + value + "')")];
      _.forEach(sequelizeModel.getFullTextIncludeModels(), function(model){
         
      });
      result.where = {$and: [this._where, $or: ]};
      result.include = sequelizeModel.getFullTextIncludeModels(); 
    } else {
