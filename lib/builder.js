...
var Model = require('../node_modules/sequelize/lib/model.js');
...
Builder.prototype.getEmbeddedIncludeModels = function(include, prefix){
  ...
  if(_.isArray(include)){
    var mappedArray = _.map(include, function(model){
      var tmpPrefix = "";
      if(model.model){
        tmpPrefix = prefix + (model.as || model.model.name); 
        if(model.limit == null){
          //TODO: fix the 'as' logic below in a better way. it should be not need model.as
          var arr = [{model: model.model, prefix: this._model.sequelize.queryInterface.quoteIdentifier(tmpPrefix) + "."}].concat(this.getEmbeddedIncludeModels(model.include, tmpPrefix + "." ));
          return arr;
        } else {
          //TODO: the line below should not be handled here as this is a undesired side effect in function 
          var res = this.getFullTextSearchClauses(model.model, true);
          model.where = res.where;
          model.include = res.include;
          return [];
        }
      }
      else{
        tmpPrefix = prefix + model.name;
        return [{model: model, prefix: this._model.queryInterface.queryInterface(tmpPrefix) + "."}]
      }
    }, this);
...
Builder.prototype.getFullTextSearchClauses = function(model, subQuery){
  var res = {where: {$or: [this.getFullTextSearchClause(model, "`" + model.name + "`" + ".")]}, include: model.getFullTextIncludeModels(model.sequelize.models)}; 
  // TODO: https://bitbucket.org/vidyahub/vidyahub/issues/42/how-does-one-pass-this-argument-to-lodash resolve
  this.getEmbeddedIncludeModels(res.include).forEach(function(model){
    if(model && model.model && model.model.getFullTextSearchColNames && _.isFunction(model.model.getFullTextSearchColNames)){
      res.where.$or.push(this.getFullTextSearchClause(model.model, model.prefix)); 
    }
  }, this);
  if(subQuery){
    var query = model.sequelize.queryInterface.QueryGenerator.selectQuery(model.tableName, res, model);
    return {where: model.sequelize.literal( "exists (select * from(" + query + ") where classId = id)"), include: null};
  } else {
    return res;
  }
}
...
Builder.prototype.build = function(sequelizeModel, options) {
  ...
  if (!_.isEmpty(this._where)) {
    ...
    if(!_.isEmpty(this._where['q'])){
      ...
      var subQuery = this._limit ? true : false;
      fullTextIncludeWhereClauses = this.getFullTextSearchClauses(sequelizeModel, subQuery);
    }
